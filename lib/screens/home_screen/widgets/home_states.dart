...
class _SearchSkeleton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      const SkeletonBox(width: double.infinity, height: 58, radius: 18);
}
...
class _ActionSkeleton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      const SkeletonBox(width: double.infinity, height: 92, radius: 22);
}
  ...
  Widget build(BuildContext context) => RefreshIndicator(
    ...
    child: ResponsiveContent(
      ...
      child: ListView(
        ...
        children: [
          Icon(Icons.cloud_off_rounded, size: 68, color: context.appMuted),
          const SizedBox(height: 20),
          DefaultText(
            text: 'home.load_error_title',
            style: TextStyle(
  ...
  Widget build(BuildContext context) => ResponsiveContent(
    ...
    child: Center(
      child: Column(
        ...
        children: [
          Icon(
            suspended ? Icons.lock_clock_rounded : Icons.phonelink_lock_rounded,
            color: suspended ? HomeColors.warning : HomeColors.brand,
            size: 72,
          ),
          const SizedBox(height: 20),
          DefaultText(
            text: suspended
                ? 'home.account_suspended'
                : 'home.job_seekers_only',
            style: TextStyle(
              color: context.appInk,
              fontSize: 22,
              fontWeight: FontWeight.w800,
          ...
          DefaultText(
            text: suspended
                ? 'home.account_suspended_body'
                : 'home.job_seekers_only_body',
            style: TextStyle(color: context.appMuted, height: 1.6),
          ),
        ],
      ),
    ),
