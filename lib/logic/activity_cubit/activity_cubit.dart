...
class ActivityCubit extends Cubit<ActivityState> {
  ...
  Map<String, dynamic> _query(int page) => {
    ...
    if (state.dateFrom != null)
      'date_from': DateFormat('yyyy-MM-dd').format(state.dateFrom!),
    if (state.dateTo != null)
      'date_to': DateFormat('yyyy-MM-dd').format(state.dateTo!),
  };
  ...
  Future<void> load({bool refresh = false}) async {
    ...
    emit(
      state.copyWith(
        ...
        refreshing: refresh && state.feed.isNotEmpty,
        loadingMore: !refresh,
        clearErrors: true,
      ),
    );
    try {
      ...
      final combined = refresh
          ? response.feed.items
          : [...state.feed, ...response.feed.items];
      final unique = <String, ActivityItem>{};
      for (final item in combined) {
        unique[item.key.isEmpty
                ? '${item.type.key}-${item.occurredAt}-${unique.length}'
                : item.key] =
            item;
      }
      emit(
        state.copyWith(
          ...
          required: response.requiresAction,
          schedule: response.upcomingSchedule,
          feed: unique.values.toList(),
          page: response.feed.meta.currentPage,
          lastPage: response.feed.meta.lastPage,
          loading: false,
          refreshing: false,
          loadingMore: false,
        ),
      ...
      emit(
        state.copyWith(
          ...
          refreshing: false,
          loadingMore: false,
          error: refresh ? _message(error) : null,
          paginationError: refresh ? null : _message(error),
        ),
      );
    }
  }
  ...
  Future<void> openItem(
    ...
    if (!item.isRead && item.notificationId != null) {
      try {
        await repo.markRead(item.notificationId!);
        _markLocal(item.notificationId!);
      } catch (_) {}
    }
    await navigate();
  }
  ...
  Future<void> markAllRead() async {
    ...
    emit(state.copyWith(markingRead: true));
    try {
      await repo.markAllRead();
      emit(
        state.copyWith(
          markingRead: false,
          summary: state.summary.copyWith(unreadNotifications: 0),
          feed: state.feed.map((e) => e.markRead()).toList(),
        ),
      );
    } catch (error) {
      emit(state.copyWith(markingRead: false, error: _message(error)));
    }
  }

  void _markLocal(int id) => emit(
    state.copyWith(
      summary: state.summary.copyWith(
        unreadNotifications: state.summary.unreadNotifications > 0
            ? state.summary.unreadNotifications - 1
            : 0,
      ),
      feed: state.feed
          .map((item) => item.notificationId == id ? item.markRead() : item)
          .toList(),
    ),
  );
