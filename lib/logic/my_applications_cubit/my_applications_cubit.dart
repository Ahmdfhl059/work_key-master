...
class MyApplicationsCubit extends Cubit<MyApplicationsState> {
  ...
  Future<void> selectGroup(String group) async {
    ...
    _requestVersion++;
    emit(state.copyWith(group: group, items: [], page: 1, lastPage: 1, clearError: true));
  ...
  Future<void> applyFilters({required List<String> statuses, required String sortBy, required String direction}) async {
    _requestVersion++;
    emit(state.copyWith(statuses: statuses, sortBy: sortBy, sortDirection: direction, items: []));
    await load(refresh: true);
  }

  Map<String, dynamic> buildQuery(int page) => {
    'group': state.group, 'sort_by': state.sortBy, 'sort_direction': state.sortDirection, 'per_page': 15, 'page': page,
  ...
  Future<void> load({bool refresh = false}) async {
    ...
    emit(state.copyWith(loading: refresh, loadingMore: !refresh, clearError: true, items: refresh ? [] : state.items));
    try {
      final response = await repo.getMyApplicationsPage(buildQuery(page));
      if (version != _requestVersion) return;
      final existing = refresh ? <JobApplication>[] : state.items;
      final merged = <int, JobApplication>{for (final item in [...existing, ...response.items]) item.id: item};
      emit(state.copyWith(items: merged.values.toList(), counts: response.meta.counts, page: response.meta.currentPage, lastPage: response.meta.lastPage, loading: false, loadingMore: false));
    } catch (_) {
      if (version == _requestVersion) emit(state.copyWith(loading: false, loadingMore: false, error: 'Unable to load your applications. Please try again.'));
    }
  }
